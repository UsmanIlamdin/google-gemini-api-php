...
class GenerativeModel
{
    ...
    public function __construct(
        private readonly Gemini $client,
     ...
     * @throws ClientExceptionInterface
     */
    public function generateContentWithCache(array $cache, PartInterface ...$parts): GenerateContentResponse
    {
        $content = new Content($parts, Role::User);

        return $this->generateCachedContentWithContents([$content], $cache);
    }
     ...
     * @throws ClientExceptionInterface
     */
    public function generateCachedContentWithContents(array $contents, array $cache): GenerateContentResponse
    {
        $this->ensureArrayOfType($contents, Content::class);

        $request = new GenerateCachedContentRequest(
            $this->modelName,
            $contents,
            $cache['cachedContent'],
            $this->safetySettings,
        ...
        return $this->client->generateContentWithCache($request);
    }
